...
pacstrap /mnt base base-devel vim grub amd-ucode linux linux-firmware linux-headers efibootmgr sof-firmware zsh plasma
...
echo 'fear' >> /etc/hostname
echo '127.0.0.1 localhost' >> /etc/hosts
echo '::1       localhost' >> /etc/hosts
echo '127.0.1.1 fear.local   fear' >> /etc/hosts
echo 'root:--' | chpasswd
useradd -m -g users -G wheel -s /bin/zsh edson
echo 'edson:--' | chpasswd
...
yes | pacman -S plasma-wayland-session dolphin dolphin-plugins gvfs gvfs-smb kfind konsole spectacle bash-completion okular gwenview kate print-manager cups system-config-printer \
...
unrar unzip p7zip mesa ark  lm_sensors i2c-tools libvdpau-va-gl libva-vdpau-driver libva-utils vdpauinfo vulkan-radeon libva-mesa-drivers mesa-vdpau mesa-utils ntfs-3g dosfstools exfat-utils btrfs-progs tailscale zerotier-one git wget curl \
...
git clone https://aur.archlinux.org/microsoft-edge-stable-bin.git /home/sylvio/AppAUR/edge
